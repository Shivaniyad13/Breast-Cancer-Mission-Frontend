...
import { recordVideoPlayAction } from "@/app/actions/analytics";
    ...
    if (videoRef.current && activeStory) {
      ...
      if (playPromise !== undefined) {
        playPromise
          .then(() => {
            setIsPlaying(true);
            recordVideoPlayAction(
              activeStory.videoUrl || activeStory.id,
              `${activeStory.fullName}: ${activeStory.storyTitle}`,
              "SURVIVOR"
            ).catch(() => {});
          })
          .catch((error) => {
